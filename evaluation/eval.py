...
if __name__ == "__main__":
    ...
    assert len(i1_jsons) == len(i2_jsons), "{}={} vs. {}={}".format(args.input1_file,len(i1_jsons),args.input2_file,len(i2_jsons))
    ...
    while i < len(message_list):
        try:
            ...
            message = "{}".format(e)
            if message.find("Azure") >= 0 and message.find("triggering") >= 0 and message.find("prompt") >= 0:
                retry = 0
                i += batch_size
                wait_base = 1
                pbar.update(batch_size)
                print("except-Azure-triggering-prompt-policy")
                print("batch_predictions:",batch_predictions)
                predictions += batch_predictions
            else:
                retry += 1
    ...
    with open(f"{output_review_f}", "w",encoding='utf-8') as fo:
        ...
        for idx, prediction in enumerate(predictions):
            ...
            review = content#prediction['choices'][0]['message']['content']
            scores = parse_score(review)
            js_dict.append(i1_jsons[idx])
            js_dict[-1]['review'] = review
